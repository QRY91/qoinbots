...
class GameState extends EventTarget {
    constructor() {
        ...
        this.state = {
            ...
            unlocks: {
                ...
                botUnlocks: {
                    'hodl-droid': { condition: 'total_losses', value: 3, unlocked: false },
                    'dip-destructor': { condition: 'total_trades', value: 10, unlocked: false },
                    'bearbot': { condition: 'loss_streak', value: 5, unlocked: false },
                    'momentum-mike': { condition: 'profit_trade', value: 5.0, unlocked: false },
                    'panic-pete': { condition: 'balance_below', value: 1.0, unlocked: false },
                    'zen-master': { condition: 'balanced_trading', value: 20, unlocked: false },
                    'fomo-frank': { condition: 'quick_trades', value: 10, unlocked: false },
                    'sage-bot': { condition: 'cycles_survived', value: 1, unlocked: false }
                },

                // Feature unlocks
                features: {
                    'bot_collection': { condition: 'first_bot_unlock', unlocked: false },
                    'create_a_bot': { condition: 'bots_collected', value: 4, unlocked: false },
                    'trading_floor': { condition: 'bots_collected', value: 6, unlocked: false },
                    'bubble_cycles': { condition: 'internal_trading', value: 100, unlocked: false }
                }
            },
            ...
            achievements: [
                ...
                { id: 'first_trade', name: 'First Steps', description: 'Make your first trade', unlocked: false },
                { id: 'broke_but_wise', name: 'Broke But Wise', description: 'Lose 90% of starting balance', unlocked: false },
                { id: 'lucky_streak', name: 'Lucky Streak', description: 'Win 5 trades in a row', unlocked: false },
                { id: 'diamond_hands', name: 'Diamond Hands', description: 'Hold through a 50% loss', unlocked: false },

                // Bot collection achievements
                { id: 'bot_collector', name: 'Bot Collector', description: 'Unlock 3 different bots', unlocked: false },
                { id: 'full_roster', name: 'Full Roster', description: 'Unlock all 8 base bots', unlocked: false },

                // Bubble cycle achievements
                { id: 'bubble_survivor', name: 'Bubble Survivor', description: 'Survive your first market crash', unlocked: false },
                { id: 'enlightened', name: 'Enlightened', description: 'Reach maximum wisdom level', unlocked: false }
            ],
    ...
    getBots() { return this.state.bots; }
    getBot(id) { return this.state.bots[id]; }
    getMarket() { return this.state.market; }
    getUnlocks() { return this.state.unlocks; }
    getAchievements() { return this.state.achievements; }
    getTradingFloor() { return this.state.tradingFloor; }
    ...
    // Statistics helpers
    getTotalTrades() {
        return Object.values(this.state.bots).reduce((total, bot) => total + bot.stats.trades, 0);
    }

    getTotalLosses() {
        return Object.values(this.state.bots).reduce((total, bot) => total + bot.stats.losses, 0);
    }

    getLongestLossStreak() {
        // Implementation would track loss streaks across all bots
        return Math.max(...Object.values(this.state.bots).map(bot => bot.lossStreak || 0));
    }

    getBiggestProfit() {
        return Math.max(...Object.values(this.state.bots).map(bot => bot.biggestProfit || 0));
    }

    getLowestBalance() {
        return Math.min(...Object.values(this.state.bots).map(bot => bot.stats.balance));
    }

    getTotalBalance() {
        return Object.values(this.state.bots).reduce((total, bot) => total + bot.stats.balance, 0);
    }
    ...
    getQuickTradeCount() {
        ...
        const activeBots = Object.values(this.state.bots).filter(bot => bot.isActive).length;
    ...
    mergeStates(defaultState, savedState) {
        ...
        if (savedState.player) {
            ...
            if (savedState.player.settings) {
                merged.player.settings = { ...merged.player.settings, ...savedState.player.settings };
            }
        }
        ...
        if (savedState.market) {
            ...
            if (savedState.market.assets) {
                merged.market.assets = { ...merged.market.assets, ...savedState.market.assets };
            }
        }
        ...
        if (savedState.unlocks) {
            ...
            if (savedState.unlocks.botUnlocks) {
                merged.unlocks.botUnlocks = { ...merged.unlocks.botUnlocks, ...savedState.unlocks.botUnlocks };
            }
        }

        if (savedState.tradingFloor) {
            merged.tradingFloor = { ...merged.tradingFloor, ...savedState.tradingFloor };
        }
    ...
    calculateOfflineProgress(lastSaved) {
        ...
        console.log(`Player was away for ${minutesAway} minutes, calculating offline progress...`);
        ...
        Object.values(this.state.bots).forEach(bot => {
            ...
            const personalityMultiplier = this.getBotOfflineMultiplier(bot.personality);
            ...
            const actualTrades = Math.min(possibleTrades, Math.floor(minutesAway / 2)); // Cap at reasonable rate
        ...
        this.emit('offline_progress_calculated', {
            ...
            tradesSimulated: Object.values(this.state.bots).reduce((sum, bot) => sum + (bot.offlineTradesCount || 0), 0)
        });
    ...
    getBotWinProbability(personality) {
        const probabilities = {
            ...
            'aggressive': 0.40,    // Risk leads to losses
            ...
            'zen': 0.60,         // Balanced approach works
