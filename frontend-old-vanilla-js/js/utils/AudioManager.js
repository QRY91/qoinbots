...
class AudioManager {
    ...
    playSound(soundName, volumeMultiplier = 1.0) {
        ...
        const effectiveVolume = this.getEffectiveVolume(soundDef.type) * volumeMultiplier;
    ...
    playSingleFrequencySound(soundDef, volume) {
        const sound = this.createOscillator(soundDef.frequency, soundDef.duration, volume);
        if (!sound) return;
    ...
    playMultiFrequencySound(soundDef, volume) {
        ...
        if (isChord) {
            ...
            frequencies.forEach((freq, index) => {
                const sound = this.createOscillator(freq, totalDuration, volume / frequencies.length);
                if (sound) {
