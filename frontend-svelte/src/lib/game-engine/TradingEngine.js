...
class TradingEngine extends EventEmitter {
  constructor(gameState) {
    ...
    this.tickRate = 250; // 250ms ticks for responsive trading
    ...
    this.cycleState = {
      ...
      phaseDurations: {
        growth: 200, // 200 ticks = ~50 seconds (was ~1.7 min)
        bubble: 100, // 100 ticks = ~25 seconds (was ~50 sec)
        peak: 20, // 20 ticks = ~5 seconds (was ~10 sec)
        crash: 60, // 60 ticks = ~15 seconds (was ~30 sec)
        recovery: 160, // 160 ticks = ~40 seconds (was ~1.3 min)
      },
    };
  ...
  restoreMarketState() {
    ...
    if (market.cycle && market.cycleProgress !== undefined) {
      ...
      this.cycleState.duration = Math.floor(
        this.cycleState.progress * maxDuration,
      );

      console.log(
        `Restored market cycle: ${this.cycleState.phase} (${Math.round(this.cycleState.progress * 100)}%)`,
      );
    }
  ...
  // Event system provided by EventEmitter base class
  ...
  updateMarketCycle() {
    ...
    const maxDuration = this.cycleState.phaseDurations[this.cycleState.phase];
    this.cycleState.progress = Math.min(
      1,
      this.cycleState.duration / maxDuration,
    );
  ...
  calculateNewPrice(asset, data) {
    ...
    const meanReversionMove =
      this.getMeanReversionMove(asset, price) * volatility;
  ...
  processBotTrading() {
    ...
    Object.values(bots).forEach((botData) => {
      console.log(`🔍 Processing bot:`, {
        ...
        balance: botData.stats?.balance,
        trades: botData.stats?.trades,
      });
      ...
      if (!this.activeBots.has(botData.id)) {
        console.log(`${botData.name}: Creating new Bot instance with data:`, {
          balance: botData.stats?.balance,
          startingBalance: botData.stats?.startingBalance,
      ...
      console.log(`${botData.name}: Bot instance stats:`, {
        balance: bot.stats.balance,
        trades: bot.stats.trades,
        lastTradeTime: new Date(bot.lastTradeTime).toLocaleTimeString(),
      });
  ...
  getTradingActivity() {
    const recentTrades = Array.from(this.activeBots.values()).reduce(
      (count, bot) => {
  ...
  popInternalBubble() {
    ...
    if (sageBot) {
      sageBot.speak(
        "I have achieved enlightenment: all these assets are illusions!",
      );
    }
  ...
  getMarketStats() {
    ...
    return {
      cycle: this.cycleState,
      totalVolume: Object.values(market.assets).reduce(
        (sum, asset) => sum + asset.volume,
        0,
      ),
      marketCap: Object.values(market.assets).reduce(
        (sum, asset) => sum + asset.price * asset.volume,
        0,
      ),
