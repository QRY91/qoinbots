...
class Bot extends EventTarget {
    constructor(config) {
        ...
        this.preferences = {
            ...
            maxPositionSize: config.maxPositionSize || 0.5,
            minTradeSize: config.minTradeSize || 1.0,
            tradingFrequency: config.tradingFrequency || 30000, // ms between trades
            ...config.preferences
        };
    ...
    shouldTrade(marketData, otherBots = []) {
        ...
        if (Date.now() - this.lastTradeTime < this.getTradeDelay()) return false;
        if (this.stats.balance < this.preferences.minTradeSize) return false;
        ...
        // Mood affects decision threshold
        const moodData = this.moods[this.mood];
        const threshold = 0.6 / moodData.tradeFrequency;
    ...
    simulateBuyTrade(size, price, asset) {
        const cost = size;
        if (this.stats.balance < cost) return 0;
    ...
    updateTradeStats(pnl, size) {
        ...
        if (pnl > 0) {
            ...
            this.stats.longestLossStreak = Math.max(this.stats.longestLossStreak, Math.abs(this.stats.currentStreak));
        }

        this.stats.winRate = this.stats.trades > 0 ? (this.stats.wins / this.stats.trades) * 100 : 0;
        this.stats.averageTradeSize = ((this.stats.averageTradeSize * (this.stats.trades - 1)) + size) / this.stats.trades;
    }
    ...
    generateTradeCommentary(trade) {
        ...
        const speeches = this.speechPatterns[context] || this.speechPatterns.trading;
    ...
    speak(message, duration = 4000) {
        if (Date.now() - this.lastSpeech < 2000) {
    ...
    calculateTradeSize() {
        ...
        return Math.max(
            this.preferences.minTradeSize,
            maxSize * moodMultiplier * personalityRisk
        );
    }
    ...
    getTradeDelay() {
        ...
        const finalDelay = (baseDelay * moodMultiplier * patientMultiplier) / 3;

        console.log(`${this.name}: Trade delay calculated - ${Math.round(finalDelay/1000)}s (base: ${Math.round(baseDelay/1000)}s)`);
        return Math.max(finalDelay, 5000); // Minimum 5 seconds between trades
    }
    ...
    analyzePriceTrend(priceChange) {
        // Different personalities react differently to price trends
        switch (this.personality) {
            case 'hodl': return 0.1; // HODL doesn't care about short-term price
            case 'dip_buyer': return priceChange < 0 ? 0.8 : 0.2;
            case 'bear': return priceChange < 0 ? 0.7 : 0.1;
            case 'momentum': return priceChange > 0 ? 0.9 : 0.1;
            default: return 0.5 + priceChange;
        }
    }

    analyzeVolume(marketData) {
        return Math.random() * 0.3; // Simplified volume analysis
    }

    getPersonalitySignal(priceChange, marketData) {
        // Personality-specific trading logic
        const signal = Math.random() * 0.5;

        // Apply personality biases
        if (this.personality === 'momentum' && priceChange > 0) return signal + 0.3;
        if (this.personality === 'dip_buyer' && priceChange < 0) return signal + 0.4;
        if (this.personality === 'bear') return signal - 0.2;
    ...
    analyzeBotBehavior(otherBots) {
        ...
        const recentTrades = activeBots.filter(bot =>
            Date.now() - bot.lastTradeTime < 60000
        ).length;
